...
const fetchCategoryNews = (id, name) => {
    ...
    fetch(url)
        ...
        // call showAllNews
        .then(data => {
           // Show Trending data with click btn for store empty Array
         fetchData = data.data; 
            
            showAllNews(data.data, name)})
}
...
const showAllNews = (id1, name1) => {
    ...
    <p class="card-text">
            <div class='d-flex align-items-center'>
                ...
                <div>
                authorName:${singleNews.author.name ? singleNews.author.name : "Ore BaBA" }
            ...
            <div class="d-flex align-items-center">
                <i class="fas fa-eye"></i>
                    ...
                    <p class="m-0 p-0 text-danger">Title-view:${singleNews.total_view ? singleNews.total_view : "Raihan RAsul"}</p>
            </div>
...
const showFetchNewsDetail = newsDtail =>{
    console.log(newsDtail.others_info.is_trending)
    const setInnerHtml= document.getElementById('modal-body');
setInnerHtml.innerHTML= `
<div class="row g-0">
<div class="col-md-12">
<img src="${newsDtail.image_url}" class="img-fluid rounded-start" alt="...">
</div>

        <div class="col-md-12 d-flex flex-column p-2">

            <div class="card-body">
                <h5 class="card-title">Title:${newsDtail.title} <span class="badge text-bg-warning">${newsDtail.others_info.is_trending ? 'Tranding' : "Min Mina MIN"}</span></h5>
                <p class="card-text">${newsDtail.details}</p>


    <div class='d-flex justify-content-between align-items-center'>
        <p class="card-text">
            <div class='d-flex align-items-center'>
                <img src="${newsDtail.author.img}" height='40' width='40' alt="" class=rounded-circle>
                <div>
                authorName:${newsDtail.author.name}
                <div>Date:${newsDtail.author.published_date}</div>
                </div>
            </div>
            ...
            <div class="d-flex align-items-center">
                <i class="fas fa-eye"></i>
                    ...
                    <p class="m-0 p-0">Title-view:${newsDtail.total_view}</p>
            </div>
           ...
           <p>rating:${newsDtail.rating.number}</p>
        </div>
        
        <div>
        <i class="fas fa-arrow-right" onclick="fetchNewsDetail('${newsDtail._id}')" data-bs-toggle="modal" data-bs-target="#exampleModal"></i>
        </div>
        </p>
    </div>
...
// step-4
let category_name=document.getElementById('category-name').innerText
// //step2 call showAllNews
// showAllNews(fetchData, "random");
//step5 call showAllNews
